...
function PagePagar() {
  ...
  const sumTotal = () => {
    ...
    const sum = productsBuyed.reduce(reducer, 0).toFixed(2);
    return sum;
  };
  const months = [
    {
  ...
  return (
    <>
      <div className="text-center">
        <h2 className="text-2xl font-semibold">Productos elegidos</h2>
        <table className="text-center mx-auto">
          <tr>
            <td className="p-2 border-2 border-green-900">Nombre</td>
            <td className="p-2">Precio</td>
            <td className="p-2">Cantidad</td>
          </tr>
          {productsBuyed.map((producto) => (
            <tr>
              <td className="p-2">{producto.name}</td>
              <td className="p-2">S/.{producto.price}</td>
              <td className="p-2">{producto.quantity}</td>
            </tr>
          ))}
        </table>
        <h2 className="text-xl font-semibold">
          Total a pagar : s./{sumTotal()}
        </h2>
      </div>
